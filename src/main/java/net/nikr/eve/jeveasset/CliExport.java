			...
			for (ExportSettings exportSettings : entry.getValue()) {
				switch (tool) {
					...
					case MARKET_ORDERS:
						...
						break;
					case MATERIALS:
						ok = export(new MaterialsData(profileManager, profileData).getData(CliOptions.get().getMaterialsOwner(), true, CliOptions.get().isMaterialsOre(), CliOptions.get().isMaterialsPI()), TableFormatFactory.materialTableFormat(), toolName, exportSettings);
						break;
					case MINING:
