...
import net.nikr.eve.jeveasset.gui.dialogs.update.UpdateDialog.CitadelTask;
...
public class BackgroundUpdate {
	...
	void update() {
		...
		CitadelTask citadelTask = new CitadelTask();
		citadelTask.update();
		...
		for (Profile profile : profileManager.getProfiles()) {
			...
			updateTasks.add(new Step2Task(profileManager, true, true, true, true, true, true, true, true));
